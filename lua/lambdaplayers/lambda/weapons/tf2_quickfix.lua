        ...
        OnThink = function( self, wepent, isdead )
            ...
            elseif !isdead and IsValid( healTarget ) then
                ...
                if CurTime() > wepent.l_HealGiveTime then
                    LAMBDA_TF2:GiveHealth( healTarget, 1, LAMBDA_TF2:GetMaxBuffedHealth( healTarget, 1.25 ) )
                    ...
                    local healRate = LAMBDA_TF2:GetMediGunHealRate( self, healTarget )
                    if self.l_TF_Medigun_ChargeReleased then 
                        healRate = ( healRate / 3 )

                        LAMBDA_TF2:GiveHealth( self, 1, LAMBDA_TF2:GetMaxBuffedHealth( self, 1.2 ) )
                    ...
                    wepent.l_HealGiveTime = ( CurTime() + healRate )
                end

                if !self.l_TF_Medigun_ChargeReleased then
                    local chargeRate = 2.75

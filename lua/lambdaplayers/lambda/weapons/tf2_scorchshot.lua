...
local function OnFlareTouch( flare, ent )
    ...
    elseif critType == TF_CRIT_MINI or LAMBDA_TF2:IsBurning( ent ) then
    ...
    if IsValid( owner ) and IsValid( ent ) and LAMBDA_TF2:IsValidCharacter( ent ) then
        ...
        angRotation[ 1 ] = ( angRotation[ 1 ] * ( random( 1, 2 ) == 1 and 1 or -1 ) )
        angRotation[ 2 ] = ( angRotation[ 2 ] * ( random( 1, 2 ) == 1 and 1 or -1 ) )
        angRotation[ 3 ] = ( angRotation[ 3 ] * ( random( 1, 2 ) == 1 and 1 or -1 ) )
    ...
    if flare.l_DidImpact then
        ...
        if IsValid( owner ) then
            ...
            dmginfo:SetAttacker( owner )
            dmginfo:SetInflictor( owner:GetWeaponENT() )
            dmginfo:SetDamageType( dmgType + DMG_HALF_FALLOFF )

            ...
            if !isdead then
                ...
                if wepent.l_TF_IsCharging then 
                    ...
                    if self:InCombat() then
                        ...
                        wepent.l_TF_ChargeTimeRequired = LAMBDA_TF2:RemapClamped( distMap, 0, 1, 0.5, ( LambdaRNG( 3.3, 4, true ) * timeMult ) )
                    else
        ...
        OnAttack = function( self, wepent, target )
            ...
            bulletTbl.Callback = function( attacker, tr, dmginfo )
                ...
                if chargeStartTime > 0.2 and tr.HitGroup == HITGROUP_HEAD then 
                    dmginfo:SetDamageCustom( dmginfo:GetDamageCustom() + TF_DMG_CUSTOM_HEADSHOT + TF_DMG_CUSTOM_DECAPITATION ) 
                end
            end
